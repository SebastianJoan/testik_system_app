...
<div class="flex justify-center items-center h-screen ">
    <div class="w-96 bg-white rounded-lg shadow-lg p-6 border border-blue-500 ">
        <div class="font-medium text-lg mb-4">Creación de Responsable</div>
            <form action="{{route('responsable.store')}}" method="POST">
                @csrf
                @METHOD('POST')
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Cargo</label>
                    <select name="id_cargo" class="form-input w-full bg-white border-b border focus:outline-none px-2 py-2 border-blue-800 rounded-sm">
                        <option value="">-- Por favor seleccciona un Cargo --</option>
                        @foreach($cargos as $data)
                            <option value="{{$data -> id_cargo}}">{{$data -> cargo}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Nombre</label>
                    <input type="text" name="nombre" placeholder="Nombre del responsable" class="form-input w-full bg-white border-b border focus:outline-none px-2 py-2 border-blue-800 rounded-sm"  required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Imagen</label>
                    <input  name="urlImage"  accept="image/png, image/gif, image/jpeg" placeholder="Nombre del responsable" class="form-input w-full  bg-white border-b  focus:outline-none px-2 py-2  rounded-sm border-2 file:bg-blue-800 file:px-2 file:py-2 file:text-white file:outline-none file:border-none cursor-pointer file:cursor-pointer file:rounded-lg" aria-describedby="file_input_help" id="file_input" type="file" >
                </div>
                <div class="mt-6">
