        ...
        <div class="font-medium text-lg mb-4">Creación de Componente</div>
            <form action="{{route('componente.store')}}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="id_servicios" class="form-input w-full bg-white border-b border focus:outline-none px-2 py-2 border-blue-800 rounded-sm">
                        <option value="">-- Por favor seleccciona un Servicio --</option>
                        @foreach($servicios as $data)
                            <option value="{{$data -> id_servicios}}">{{$data -> id_servicios}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
