    ...
    <div class="w-full h-auto flex flex-col px-2 md:px-28 py-4 ">
        ...
        <div class="bg-gray-100 border-2 border-gray-800 px-2 h-auto w-full rounded-b-lg flex justify-center items-start justify-items-center">
            <div class="w-1/3 px-2 py-2 flex flex-col gap-2">
                @foreach($data -> componentes_servicio  as $componentes )
                    COMPONENTE: {{ $componentes -> nombre }}
                @endforeach
                <a href="{{route('componenteServicio.servicio.new',$data -> id_servicios)}}" class=" w-full py-2 bg-green-500 rounded-md font-bold text-white hover:bg-green-700 hover:transition-colors">
                    <button class="w-full">
                        Asignar Componente
                    </button>
                </a>
